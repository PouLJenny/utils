...
public class EntityUtils {
    ...
    public static <T> void setCreateInfo(T entity) {
        ...
        String name = "";
        String id = "";
        String[] fields = { "crtName", "crtUser", "crtHost", "crtTime", "createTime", "creatorId" };
        Field field = ReflectionUtils.getAccessibleField(entity, "createTime");
        // 默认值
        Object[] value = null;
        if (field != null && field.getType().equals(Date.class)) {
            Date now = Date.from(Instant.now());
            value = new Object[] { name, id, hostIp, now, now, id };
        }
        // 填充默认属性值
    ...
    public static <T> void setUpdatedInfo(T entity) {
        ...
        String id = "";
        // 默认属性
        String[] fields = { "updName", "updUser", "updHost", "updTime", "modifyTime" };
        Field field = ReflectionUtils.getAccessibleField(entity, "modifyTime");
        Object[] value = null;
        if (field != null && field.getType().equals(Date.class)) {
            Date now = Date.from(Instant.now());
            value = new Object[] { name, id, hostIp, now, now };
        }
        // 填充默认属性值
        setDefaultValues(entity, fields, value);
    }
