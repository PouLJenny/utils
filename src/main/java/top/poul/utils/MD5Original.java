 ...
 * 
 * @author Poul.Y
 * @date 2016 上午10:01:52
 * @version 1.0
 * @since 1.0
 */
public class MD5Original {
	...
	private static long[] result = { A, B, C, D };// 存储HASH结果，共4*32 =
													// 128位，初始化值为（幻数的级联）

	public static String digest(byte[] inputBytes) {

		int byteLength = inputBytes.length;// 长度（字节）
		int groupCount = 0;// 完整分组的个数
		groupCount = byteLength / 64; // 每组512位（64字节）
		...
		// 处理完整分组后的尾巴
		int rest = byteLength % 64;// 512位分组后的余数
		byte[] tempBytes = new byte[64];
		if (rest <= 56) {
			for (int i = 0; i < rest; i++)
				tempBytes[i] = inputBytes[byteLength - rest + i];
			if (rest < 56) {
	...
	public static void main(String[] args) {
		try {
			System.out.println(digest("Hello!".getBytes("UTF-8")));
